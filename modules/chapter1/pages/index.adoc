...
Let's apply our knowledge to a concrete scenario: 

You need to serve granite-3.3-8b-instruct that is integrated with a chat application. Your primary goal is to minimize inference latency, the time it takes for a single request to complete for a given number of concurrent users:

 *  32 users concurrently. 
 *  With an expected generation length < 2048 tokens. 
